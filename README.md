...
현재 실행 가능한 기준선은 v0.1이다. v0.1은 "문장형 문법 + 엄격한 내부 구조 + 작은 실행 가능 범위"를 우선한다. v0.2는 그 위에서 메시지 프레임 확장 규칙을 정리하는 연구 단계이며, 버전별 구분은 `HISTORY.md`에 정리한다.
...
- [HISTORY.md](./HISTORY.md): `v0.1`과 `v0.2` 단계 구분과 버전별 변화 요약
