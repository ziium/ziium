...
안정된 코어 문법 위에 몇 가지 실험 프레임도 함께 들어가 있다. 현재 실험 프레임은 `X으로 Y`, `X 더하기 Y`, `<대상>에 <값> 동작`, `<함수>를 <레코드>로 호출한다`, `...에서 맨위 요소를 꺼낸 것이다`, `...에서 맨위 요소를 꺼낸다` 같은 형태다. 이들은 모두 닫힌 built-in 집합이나 제한된 위치에서만 동작한다.
