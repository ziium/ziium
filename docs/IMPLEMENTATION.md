...
표면 AST는 여전히 `Call`, `TransformCall`, `Property`, `KeywordMessage`, `Resultive`처럼 surface syntax 중심으로 나뉜다. 여기서 `Resultive`는 바인딩 RHS 식과 statement-only effect frame을 표면에서 구분한 채 유지한다. 그 다음 HIR lowering이 이를 `Send` 중심 표현으로 다시 정리한다.
