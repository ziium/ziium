...
### P17B. built-in처럼 보이는 이름도 속성 접근으로 먼저 파싱
...
- 두 우변 모두 `PropertyExpr`
- `상자의 길이`는 `PropertyExpr(NameExpr("상자"), "길이")`
- `숫자의 제곱`은 `PropertyExpr(NameExpr("숫자"), "제곱")`
...
- `ParseSuccess`
- property chain receiver와 함께 현재 닫힌 effect frame `...에서 맨위 요소를 꺼낸다`를 문장으로 파싱
...
- 바인딩 RHS와 문장 자리 모두에서 `맨뒤 요소` 결과 서술 파싱 성공
- `role_label`은 `맨뒤 요소`
...
- 바인딩 RHS와 문장 자리 모두에서 `맨앞 요소` 결과 서술 파싱 성공
- `role_label`은 `맨앞 요소`
...
- `ParseError`
- 현재 닫힌 위치형 `꺼내기` 범위 밖 role label은 거부
